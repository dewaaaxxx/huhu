...
namespace AutoPlay {
    ...
    void ScanFast() {
        ...
        static const double kAngleOffsets[] = {
            0.0, -0.005, +0.005, -0.0105, +0.0105,
            -0.0175, +0.0175, -0.026, +0.026, -0.035, +0.035
        };
        static const double kPowerFactors[] = {
            1.0, 1.05, 0.95, 1.12, 0.88, 1.2, 0.85,
            1.35, 0.72, 1.5, 0.6, 1.75, 0.45, 2.0, 0.35
        };
        ...
        for (const auto& cand : candidates) {
            ...
            for (double dA : kAngleOffsets) {
                ...
                for (double pf : kPowerFactors) {
                    ...
                    // 🔥 Panggil simulasi dengan angle & power yang dicoba
                    ...
                    if (isNineBall) {
                        ...
                        validShots.push_back({c9, confirmedAngle, confirmedPower, cand.score, 0});
                        simOk = true;
                        break;
                    }
    
                    // 8-ball: coba power yang lebih halus
                    double bestPower = tryPower;
                    const double pTry[3] = { tryPower * 0.70, tryPower * 0.82, tryPower * 0.92 };
                    for (int pi = 0; pi < 3; pi++) {
                        double rp = pTry[pi];
                        if (rp < 80.0) rp = 80.0;
                        if (rp >= bestPower) continue;
                        gPrediction->determineShotResult(true, tryAngle, rp, sharedGameManager.getShotSpin(), cand);
                    ...
                    confirmedAngle = tryAngle;
                    confirmedPower = bestPower;
                    validShots.push_back({cand, confirmedAngle, confirmedPower, cand.score, 0});
                    simOk = true;
                    break;
                }
                if (simOk) break;
